...
//            for (int i = 0; i < elements1.first().select("tr").size(); i++) {
//                HashMap rowheadsdt = new HashMap(); // 行信息
//                HashMap colheadsdt = new HashMap(); // 列信息
//
//                // 获取明细表的宽度，border，padding
//                Integer widthdt = 0;
//                if (elements1.get(i).attr("style").split(":")[1].endsWith("%")) { // 如果是百分比形式，进行特殊处理
//                    widthdt = Integer.valueOf(elements1.get(i).attr("style").split(":")[1].split("%")[0].trim()) / 100 * width - border * 2 - padding * 2;
//                } else {
//                    widthdt = Integer.valueOf(elements1.get(i).attr("style").split(":")[1].trim()); // 不是百分比形式，直接取值
//                }
//                Integer borderdt = Integer.valueOf(elements1.get(i).attr("border").trim().equals("") ? "0" : elements1.get(i).attr("border").trim());
//                Integer paddingdt = Integer.valueOf(elements1.get(i).attr("padding").trim().equals("") ? "0" : elements1.get(i).attr("padding").trim());
//                Elements midTable1td1 = elements1.get(i).select("td.midTable1td1");
//                Elements midTable1td2 = elements1.get(i).select("td.midTable1td2");
//                Integer colwidthdt = (widthdt - borderdt * 2 - paddingdt * 2) / midTable1td1.size();
//
//                // 每一列
//                String colspan = ""; // 行单元格合并
//                String rowspan = ""; // 列单元格合并
//                for (int j = 0; j < midTable1td1.size(); j++) {
//
//                    HashMap cell = new HashMap();
//                    rowheadsdt.put("row_" + j, 30); // 顺序不是按照put的先后顺序排列...
//                    colheadsdt.put("col_" + j, colwidthdt);
//
//                    // 对colspan和rowspan之后的单元格定位进行处理
//                    cell.put("id", i + "," + (j + (colspan.equals("") ? 0 : Integer.valueOf(colspan))));
//                    colspan = elements1.select("tr").select("td").get(i * j + j).attr("colspan").trim();
//
//                    // 列合并比较复杂，先不考虑，假设所有单元格都没有列合并
////                    rowspan = elements1.select("tr").select("td").get(i * j + j).attr("rowspan").trim();
//                    cell.put("colspan", colspan.equals("") ? 0 : Integer.valueOf(colspan));
//                    cell.put("rowspan", 0);
//                    cell.put("etype", elements1.first().select("td").attr("td.InputStyle"));
//                    ecdt.add(j, cell);
//                }
////                for (int j = 0; j < midTable1td2.size(); j++) {
////                    rowheadsdt.put("row_" + j, 30);
////                    colheadsdt.put("col_" + j, colwidthdt);
////                }
//
...
//                etables.put("detail_" + (i + 1), detailtable);
//
//            }
//
//            // 将主表和明细表的信息放到总表单内
